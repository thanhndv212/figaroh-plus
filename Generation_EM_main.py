...
from tools.trajectory_functions_def import (
    generate_trapezoidal_axis_motion,
    generate_optimal_excitation_motion,
)
...
def main():
    ...
    robot = load_model("hidro_arm_2dof_urdf", "hidro_arm_2dof.urdf", params_settings)
    ...
    if params_settings["SAVE_FILE"]:
        path_results = "results/trajectory/" + model.name
    ...
    # in the URDF
    params_settings["q0"] = np.array(robot.q0)

    set_missing_params_setting(robot, params_settings)
    ...
    # 2. Assuming we have has_friction we will generate using trapezoidal motions joint
    # by joint EM for identifting the motor  and segment inertia and joint friction
    q = np.empty(model.nq)
    dq = np.empty(model.nq)
    ddq = np.empty(model.nq)
    print(int(100 / params_settings["trapez_vel_steps"]))
    for vel_step in range(10):  # int (100/params_settings['trapez_vel_steps']) ):
        trapez_vel = np.zeros(robot.model.nq)
        for ii in range(robot.model.nq):
            trapez_vel[ii] = (
                robot.model.velocityLimit[ii]
                * (vel_step + 1)
                * (params_settings["trapez_vel_steps"] / 100)
            )
        params_settings["trapez_vel"] = trapez_vel

        print(params_settings["trapez_vel"])
        q_all, dq_all, ddq_all = generate_trapezoidal_axis_motion(
            robot, params_settings
        )
        q = np.vstack((q, q_all))
        dq = np.vstack((dq, dq_all))
        ddq = np.vstack((ddq, ddq_all))

    time_traj = np.linspace(0, len(q) * params_settings["ts"], len(q))
    ...
    nb_samples = len(q)
    # update teh regressor matrix using new joint trajectory
    W = build_regressor_basic(robot, q, dq, ddq, params_settings)
    ...
    # calculate the joint torque from identified base parameters
    # tau_iden = np.matmul(W_base, phi_base)
    ...
    if params_settings["SAVE_FILE"]:
        header = "Exciting trapezoidal trajectories"
        # header += "q1, q1, dq1, dq2"
        data_save = np.c_[time_traj, q, dq, ddq]
        np.savetxt(
            path_results + "/joint_by_joint_trapezoidal_trajectory.csv",
            data_save,
            header=header,
            delimiter=",",
        )

    # 3.  Generate Optimal Exciting Motions (OEM) to excite all remaining parameters

    # 3.1 if param['is_static_regressor'] is true then we will
    # - build the static regressor
    # - excite only the static parameters,i.e  a serie of exciting postures will be
    # generated and linked with polynomial interpolation

    if params_settings["is_static_regressor"]:
        q = np.random.uniform(
            low=-6, high=6, size=(10 * params_settings["nb_samples"], nq)
        )
        dq = np.zeros((10 * params_settings["nb_samples"], nq))
        ddq = np.zeros((10 * params_settings["nb_samples"], nq))
    else:
        q = np.random.uniform(
            low=-6, high=6, size=(10 * params_settings["nb_samples"], nq)
        )
    ...
    # get the idx_base, ie. the index of base parameters in the initial regressor matrix
    # W
    _, params_base_static, idx_base_static = get_baseParams(
        W_e, params_r, params_standard
    )
    # save indexes of regressor's column to be used in the optimizer
    params_settings["idx_base"] = idx_base_static

    if params_settings["is_static_regressor"]:
        print("The structural static base parameters are: ")
    else:
        print("The structural static base parameters are: ")
    for ii in range(len(params_base_static)):
        print(params_base_static[ii])

    if params_settings["is_static_regressor"]:
        params_settings["idx_base_static"] = idx_base_static

    # set to true so that all joints moves from one posture to another at the same time
    params_settings["sync_joint_motion"] = True
    q, dq, ddq = generate_optimal_excitation_motion(robot, params_settings)
    time_traj = np.linspace(0, len(q) * params_settings["ts"], len(q))

    if params_settings["SAVE_FILE"]:
        header = "Exciting postures trajectories"
        # header += "q1, q1, dq1, dq2"
        data_save = np.column_stack((time_traj, q, dq, ddq))
        np.savetxt(
            path_results + "/optimal_exciting_postures_trajectory.txt",
            data_save,
            header=header,
            delimiter=",",
        )
    ...
    # plt.show()
    nb_samples = len(q)
    # update the regressor matrix using new OEM
    W = build_regressor_basic(robot, q, dq, ddq, params_settings)
    # select only the columns of the regressor corresponding to the structural base
    # parameters
    W_base = W[:, idx_base_static]
    print("When using OEP cond num is", int(np.linalg.cond(W_base)))
    ...
    # Least-square identification process
    phi_base_static = np.matmul(np.linalg.pinv(W_base), tau_meas)

    print("The values of static base parameters are:")
    print(phi_base_static)

    # calculate the joint torque from identified base parameters
    # tau_iden = np.matmul(W_base, phi_base_static)
    ...
    # 3.2 if param['is_inertia_regressor'] is true then we will
    # - build the inertia regressor
    # - excite only the inertia parameters,i.e  a serie of exciting postures will be
    # generated and linked with polynomial interpolation

    params_settings["is_static_regressor"] = False
    # We will extract only the base parameters index corresponding to the inertia
    # parameters, i.e. not the friction and the static ones
    idx_friction = []
    for ii in idx_fv:  # Two friction parameters per joint
        idx_friction.append(idx_base[ii])

    for ii in idx_fs:  # Two friction parameters per joint
        idx_friction.append(idx_base[ii])

    idx_base_inertia = idx_base
    for ii in range(len(idx_base_static)):
        idx_base_inertia.remove(idx_base_static[ii])
    print(idx_base_inertia)

    for ii in range(len(idx_friction)):
        print(idx_friction[ii])
        idx_base_inertia.remove(idx_friction[ii])

    params_settings["idx_base"] = idx_base_inertia
    ...
    if params_settings["SAVE_FILE"]:
        header = "Exciting inertia trajectories"
        # header += "q1, q1, dq1, dq2"
        data_save = np.column_stack((time_traj, q, dq, ddq))
        np.savetxt(
            path_results + "/optimal_exciting_motions_trajectory.txt",
            data_save,
            header=header,
            delimiter=",",
        )
    ...
    nb_samples = len(q)
    # update teh regressor matrix using new OEM
    W = build_regressor_basic(robot, q, dq, ddq, params_settings)
    # select only the columns of the regressor corresponding to the structural base
    # parameters
    W_base = W[:, idx_base_inertia]
    print("When using OEM cond num is", int(np.linalg.cond(W_base)))
    print(W_base.shape)
    ...
    for ii in range(nb_samples):
        ...
        for j in range(model.nq):
                tau_meas[j*nb_samples + ii] = tau_temp[j]
                #tau_meas[j*nb_samples + ii] = tau_meas[j*nb_samples + ii]
                #+ dq[ii, j]*param['fv'][j] + np.sign(dq[ii, j])*param['fs'][j]
    ...
    # W_b_u, numrank_W_u, params_rsorted_u,phi_b_u =
    # double_QR(tau_u, W_e_u, params_r_u)

    #

    # print(param_r)
    # W_b_u, phi_b_u, numrank_W_u, params_rsorted_u =
    # QR_pivoting(W, tau_trap, params_r_u)
    ...
    # coeff = coeff = 0.01 * np.ones(
    # (2 * param["NbH"] + 1, model.nq)
    ...
    #W_b_u, phi_b_u, numrank_W_u, params_rsorted_u =
    #QR_pivoting(W_e_u, tau_u, params_r_u)
