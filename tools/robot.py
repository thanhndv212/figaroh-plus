...
def load_model(robotname, robot_urdf_file, param):
    ...
    if not param["is_external_wrench"]:
...
class Robot(RobotWrapper):
    def __init__(
        ...
        if not isFext:
            ...
            self.initFromURDF(urdf_model_path, mesh_dir, pin.JointModelFreeFlyer())
    ...
    def get_standard_parameters(self):
        """This function prints out the standard inertial parameters defined in urdf
        model.
