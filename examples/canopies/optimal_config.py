...
class Detmax():
    ...
    def main_algo(self):
        ...
        while opt_k != rm_j: 
            # add one sample that maximize the increase critD(N->N+1)
            ...
            opt_critD = self.get_critD(cur_set)
            # remove one sample that miminize the decrease of critD(N+1->N)
...
# Load project configuration into dictionary
...
side = 'left'
input_file = f'data/canopies_calibration_{side}_500.yaml'
input_file = f'data/canopies_calibration_{side}_joint_configurations_test_copy.yaml'
...
for i in range(len(df)):
    for j, name in enumerate(q_jointNames):
        jointidx_q = rank_in_configuration(model, name)
        q[i, jointidx_q] = df[name][i]

# update number of data points
param['NbSample'] = len(df)

# calculate the regressor
...
for ii, param_b in enumerate(param['param_name']):
    print(ii+1, param_b)

# required minimum number of configurations
if param['calib_model'] == 'full_params':
    min_NbChosen = int(len(param['actJoint_idx'])*6/param['calibration_index']) + 1
elif param['calib_model'] == 'joint_offset':
    min_NbChosen = int(len(param['actJoint_idx'])/param['calibration_index']) + 1
