...
def sig_filter(sig, med_fil=5, nbutter=4, f_butter=2, f_sample=120):
...
for i in range(q_abs.shape[1]):
    q_abs_res[:, i] = sig_resample(q_abs[:, i], t_q, len(t_q), f_q)
    q_pos_res[:, i] = sig_resample(q_pos[:, i], t_q, len(t_q), f_q)

# marker data
should_pose = extract_tf(path_to_tf, frame_names=['"world"'])
f_tf = 120

t_tf = should_pose['"world"'][:, 0]
posX = should_pose['"world"'][:, 1]
posY = should_pose['"world"'][:, 2]
posZ = should_pose['"world"'][:, 3]
quatX = should_pose['"world"'][:, 4]
quatY = should_pose['"world"'][:, 5]
quatZ = should_pose['"world"'][:, 6]
quatW = should_pose['"world"'][:, 7]

# filter and resample
posX_butf = sig_filter(posX, med_fil=5, nbutter=4, f_butter=2, f_sample=120)
posY_butf = sig_filter(posY, med_fil=5, nbutter=4, f_butter=2, f_sample=120)
posZ_butf = sig_filter(posZ, med_fil=5, nbutter=4, f_butter=2, f_sample=120)
quatX_butf = sig_filter(quatX, med_fil=5, nbutter=4, f_butter=2, f_sample=120)
quatY_butf = sig_filter(quatY, med_fil=5, nbutter=4, f_butter=2, f_sample=120)
quatZ_butf = sig_filter(quatZ, med_fil=5, nbutter=4, f_butter=2, f_sample=120)
quatW_butf = sig_filter(quatW, med_fil=5, nbutter=4, f_butter=2, f_sample=120)

posX_res = sig_resample(posX_butf, t_tf, len(t_q), f_q)
posY_res = sig_resample(posY_butf, t_tf, len(t_q), f_q)
posZ_res = sig_resample(posZ_butf, t_tf, len(t_q), f_q)
quatX_res = sig_resample(quatX_butf, t_tf, len(t_q), f_q)
quatY_res = sig_resample(quatY_butf, t_tf, len(t_q), f_q)
quatZ_res = sig_resample(quatZ_butf, t_tf, len(t_q), f_q)
quatW_res = sig_resample(quatW_butf, t_tf, len(t_q), f_q)
