...
# 20230912
folder = '20230912/selected_suspension/sinu_motion_around_z_weight_2023-09-12-16-44-18'
# folder = '20230912/selected_suspension/sinu_motion_around_z_weight_2023-09-12-16-41-48'
# folder = '20230912/selected_suspension/sinu_motion_around_x_fold_weight_2023-09-12-16-46-52'
...
dir_path = '/home/thanhndv212/Downloads/experiment_data/suspension/bags/' + folder + '/'
...
posXYZ_res, quatXYZW_res = get_XYZQUAT_marker('shoulder', path_to_tf, t_res, f_res, f_cutoff)
posXYZ_ee, quatXYZW_ee = get_XYZQUAT_marker('gripper', path_to_object, t_res, f_res, f_cutoff)
posXYZ_base, quatXYZW_base = get_XYZQUAT_marker('base', path_to_base, t_res, f_res, f_cutoff)

rpy_shoulder = convert_quat_to_rpy(quatXYZW_res)
rpy_ee = convert_quat_to_rpy(quatXYZW_ee)
rpy_bse = convert_quat_to_rpy(quatXYZW_base)

########################################################################

# processed data
t_sample = t_res[sample_range]
q_arm, dq_arm, ddq_arm = calc_vel_acc(robot, q_abs_res, sample_range, joint_names, f_res, f_cutoff)
q_marker, dq_marker, ddq_marker, rpy_marker, Mmarker0_mocap, q_marker_ext = calc_fb_vel_acc(posXYZ_res, quatXYZW_res, sample_range, f_res)
q_markerBase, dq_markerBase, ddq_markerBase, rpy_markerBase, Mmarker0_mocapBase, q_marker_extBase = calc_fb_vel_acc(posXYZ_base, quatXYZW_base, sample_range, f_res)
...
var_init_fb[0:3] = np.array([0., 0., 0.23])
...
LM_solve = least_squares(cost_function_fb, var_init_fb, method='lm', verbose=1, \
                        args=(tiago_fb, Mmarker0, q_m, q_arm, dq_arm, ddq_arm, \
                        f_res, sol_found , UNKNOWN_BASE, base_input))

print(LM_solve)

# # estimate from solution
tau_predict_vector, tau_mea_vector, q_base, dq_base, ddq_base, rpy, reg_matrix \
    = cost_function_fb(LM_solve.x, tiago_fb, Mmarker0, q_m, q_arm, dq_arm, \
                       ddq_arm, f_res, True, UNKNOWN_BASE, base_input)
error_tau = (tau_predict_vector - tau_mea_vector).reshape((NbSample,6))
...
Mbase_marker = convert_XYZQUAT_to_SE3norm(np.concatenate((base_xyz, base_quat), axis=0))
