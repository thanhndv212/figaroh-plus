...
class CubicSpline:
    ...
    def get_active_config(self,
        ...
        # dimensions
        assert (
            self.dim_q == waypoints.shape), "(Pos) Check size \
        ...
        if vel_waypoints is not None and acc_waypoints is not None:
            # dimensions
            assert (
                self.dim_v == vel_waypoints.shape), "(Vel) Check size\
                                        (num_active_joints, num_waypoints)!"
            assert (
                self.dim_v == acc_waypoints.shape), "(Acc) Check size\
                                        (num_active_joints, num_waypoints)!"
...
class WaypointsGeneration(CubicSpline):
    ...
    def gen_rand_pool(self, soft_limit_pool=None):
        ...
        assert np.array(soft_limit_pool).shape == (3,len(self.act_idxq)), \
        ...
        for i in range(len(self.act_idxv)):
            ...
            new_upper_ddq[i] = k * (self.upper_dq[i] - lim_ddq[i] * 
                abs(self.upper_dq[i] - self.lower_dq[i]))  # Fixed line break
...
def main():
    ...
    assert len(T) == num_waypoints, "number of timestamps to be set equal to\
        number of waypoints"
    soft_lim = 0.1
