...
q_arm, dq_arm, ddq_arm = calc_vel_acc(robot, q_pos_res, selected_range, joint_names, f_res, f_cutoff)
q_arm = q_pos_res[:, actJoint_idx]
...
for base in ['base1', 'base2']:
    for gripper in ['gripper1', 'gripper2', 'gripper3']:      
        ...
        save_selected_data(active_joints, gripper_pos, q_sel, join(dirname(str(abspath(__file__))), "data/vicon_calibration_fc10_{}_{}.csv".format(gripper, base)))
        save_selected_data(active_joints, gripper_shoulder, q_sel, join(dirname(str(abspath(__file__))), "data/vicon_calibration_fc10_{}_shoulder.csv".format(gripper)))
        save_selected_data(active_joints, shoulder_base, q_sel, join(dirname(str(abspath(__file__))), "data/vicon_calibration_fc10_shoulder_{}.csv".format(base)))
