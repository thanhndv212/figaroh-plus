...
# TODO:
# - generalize for cases i.e. mocap
# - create base RobotCalibration
# - create a generic form for robot_config.yaml
class RobotCalibration:
    def __init__(self, robot, config_file, del_list=[]):
        ...
        # initiate project settings parameters
        self.param = None
    ...
    def initialize(self):
        """ Loads data experimental data set and creates a list of paramater
        of the model.
        """
        self.load_data_set()
    ...
    def cost_function(self, var):
        """
        A 1D vector of residuals between measures and model predicted outcomes.
        """
    ...
    def solve_optimisation(self):
        """
        Solver: LM
        Initial guess: nonimated values at 1st iteration, then randomized.
        "outlier_eps": thereshold to remove onlined data point
        del_list_: contained removed data points
        iteration stops once iterating maximum count reaches
        """
        _var_0 = self.set_init_guess()
    ...
    # TODO: more informative plotting
    def plot(self, lvl=1):
        if lvl == 1:
    ...
    def plot_3d_poses(self, INCLUDE_UNCALIB=True):
        ...
        PEEe_uncalib = self.get_pose_from_measure(self.set_init_guess())
    ...
    if file_type == "xacro":
        ...
        if file_name is None:
            path_save_yaml = abspath(
                "data/calibration/identified_parameter/eye_hand/tiago_master_calibration_{}.yaml".format(
                    param["NbSample"]
                )
            )
        else:
            path_save_yaml = abspath(
                "data/calibration/identified_parameter/eye_hand/" + file_name
            )
