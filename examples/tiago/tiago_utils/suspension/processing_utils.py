...
def create_rigidbody_frame(markers=[], unit_rot=None):
    """create a rigid body frame from markers, in which:
        ...
        - the x-axis is the vector from the first element to the second
         element of markers list.
        - the xy-plane is the plane formed by the first three elements of
         markers list.
...
def extract_tf(path_to_tf, frame_names):
    ...
    Input:  path_to_tf: path to csv file
            frame_name: list of str, frame defined in Qualisys streamed and
             recorded in rosbag
    Output: a dictionary
...
def get_XYZQUAT_marker(
    marker_frame_name, path_to_tf, t_res, f_res, f_cutoff, plot=True
):
