...
from figaroh.identification.identification_tools import get_param_from_yaml,base_param_from_standard,calculate_first_second_order_differentiation, calculate_standard_parameters, low_pass_filter_data
...
for ii in range(forces_trajectories_mocap.shape[1]):
    if ii == 0 : 
        forces_trajectories_mocap_filt = low_pass_filter_data(forces_trajectories_mocap[:,ii], params_settings,5)
    else : 
        forces_trajectories_mocap_filt = np.column_stack((forces_trajectories_mocap_filt,low_pass_filter_data(forces_trajectories_mocap[:,ii], params_settings,5)))
...
for ii in range(len(q_mocap)):
    quat = pin.Quaternion(np.array([q_mocap[ii,3],q_mocap[ii,4],q_mocap[ii,5],q_mocap[ii,6]]))
...
for ii in range(rotvec_nofilt.shape[1]):
    if ii == 0 : 
        ...
        rotvec = np.column_stack((rotvec,low_pass_filter_data(rotvec_nofilt[:,ii], params_settings,5)))

quat_filt = np.zeros((rotvec.shape[0],4))

for ii in range(rotvec.shape[0]):
    rotvec_ii = rotvec[ii,:]
    quat_ref = np.array([q_mocap[ii,3],q_mocap[ii,4],q_mocap[ii,5],q_mocap[ii,6]])
...
for ii in range(model_mocap.nq):
    if ii == 0:
        ...
        q_m = np.column_stack((q_m,low_pass_filter_data(q_mocap[:,ii], params_settings,5)))
...
q_m, dq_m, ddq_m = calculate_first_second_order_differentiation(model_mocap, q_m, params_settings)
...
for ii in range(nb_samples):
    ...
    pin.updateFramePlacements(model_mocap,data_mocap)
    M_pelvis = data_mocap.oMi[model_mocap.getJointId('root_joint')]
    tau_temp = pin.Force(np.array(pin.rnea(model_mocap, data_mocap, q_m[ii,:], dq_m[ii,:], ddq_m[ii,:])[:6]))#force de réaction
    tau_temp = pin.Force(np.array([tau_temp.linear[0],tau_temp.linear[1],tau_temp.linear[2],tau_temp.angular[0],tau_temp.angular[1],tau_temp.angular[2]]))
...
phi_standard_mocap, phi_AT = calculate_standard_parameters(model_mocap, W_e_mocap, tau_meast_mocap, COM_max, COM_min,params_standard_mocap, 0.33)
...
for ii in range(len(id_inertias_mocap)):
    if ii!=0 and ii != 7 and ii != 12:
        ...
        phi_comx_est_mocap.append(phi_standard_mocap[10*ii+6]/phi_standard_mocap[10*ii+9])
...
for ii in range(len(id_inertias_mocap)):
    if ii!=0 and ii != 7 and ii != 12:
        ...
        phi_comy_est_mocap.append(phi_standard_mocap[10*ii+7]/phi_standard_mocap[10*ii+9])
...
for ii in range(len(id_inertias_mocap)):
    if ii!=0 and ii != 7 and ii != 12:
        ...
        phi_comz_est_mocap.append(phi_standard_mocap[10*ii+8]/phi_standard_mocap[10*ii+9])
...
for ii in range(len(id_inertias_mocap)):
    if ii!=0 and ii != 7 and ii != 12:
        S_AT = np.array([[0,-phi_AT[10*ii+8]/phi_AT[10*ii+9],phi_AT[10*ii+7]/phi_AT[10*ii+9]],[phi_AT[10*ii+8]/phi_AT[10*ii+9],0,-phi_AT[10*ii+6]/phi_AT[10*ii+9]],[-phi_AT[10*ii+7]/phi_AT[10*ii+9],phi_AT[10*ii+6]/phi_AT[10*ii+9],0]])
        mSTS_AT = phi_AT[10*ii+9]*S_AT.T@S_AT
        phi_ixx_AT.append(phi_AT[10*ii]-mSTS_AT[0,0])

        S_mocap = np.array([[0,-phi_standard_mocap[10*ii+8]/phi_standard_mocap[10*ii+9],phi_standard_mocap[10*ii+7]/phi_standard_mocap[10*ii+9]],[phi_standard_mocap[10*ii+8]/phi_standard_mocap[10*ii+9],0,-phi_standard_mocap[10*ii+6]/phi_standard_mocap[10*ii+9]],[-phi_standard_mocap[10*ii+7]/phi_standard_mocap[10*ii+9],phi_standard_mocap[10*ii+6]/phi_standard_mocap[10*ii+9],0]])
...
names=['l_ul','l_ll','l_f','pel','abd','tho','l_ua','l_la','l_h','hd','r_ua','r_la','r_h','r_ul','r_ll','r_f']
...
ax.bar(X-0.2, comx_est_mocap.values(), width=0.2, color='g', align='center',label='mocap')
ax.legend()
...
ax.bar(X-0.2, comy_est_mocap.values(), width=0.2, color='g', align='center',label='mocap')
ax.legend()
...
ax.bar(X-0.2, comz_est_mocap.values(), width=0.2, color='g', align='center',label='mocap')
ax.legend()
