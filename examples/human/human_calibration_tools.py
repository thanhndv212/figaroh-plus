    ...
    **Reference**
    ...
    if m > 0:
        if np.abs(OffsetAngle) > np.spacing(np.single(1)):
            ...
            if np.abs(a) * 1e8 <= np.abs(c) and np.min(
                np.abs(Solutions)
            ) < 1e-8 * np.max(np.abs(Solutions)):
                Solutions = -c / b
            else:
                a = a * 2
...
def compute_joint_centers(DMarkers):
    ...
    pel_xaxis = (DMarkers["RASI"] + DMarkers["LASI"]) / 2 - (
        DMarkers["RPSI"] + DMarkers["LPSI"]
    ) / 2
    ...
    R_pelvis = np.array(
        [
            [pel_xaxis[0], pel_yaxis[0], pel_zaxis[0]],
            [pel_xaxis[1], pel_yaxis[1], pel_zaxis[1]],
            [pel_xaxis[2], pel_yaxis[2], pel_zaxis[2]],
        ]
    )

    ASIS_ASIS = np.linalg.norm(DMarkers["RASI"] - DMarkers["LASI"])

    offset_lasis = R_pelvis @ np.array(
        [-0.22 * ASIS_ASIS, -0.3 * ASIS_ASIS, 0.14 * ASIS_ASIS]
    )
    offset_rasis = R_pelvis @ np.array(
        [-0.22 * ASIS_ASIS, -0.3 * ASIS_ASIS, -0.14 * ASIS_ASIS]
    )
    ...
    LKNEC = chord_joint_centre(
        DMarkers["LKNEW"], DMarkers["LKNE"], LHIPC, DMarkers["LTHI"]
    )
    positions.append(LKNEC)

    LANKC = chord_joint_centre(
        DMarkers["LANKW"], DMarkers["LANK"], LKNEC, DMarkers["LTIB"]
    )
    ...
    RKNEC = chord_joint_centre(
        DMarkers["RKNEW"], DMarkers["RKNE"], LHIPC, DMarkers["RTHI"]
    )
    positions.append(RKNEC)

    RANKC = chord_joint_centre(
        DMarkers["RANKW"], DMarkers["RANK"], LKNEC, DMarkers["RTIB"]
    )
...
def get_local_markers(model, data, q0, markers_global):
    ...
    local_values.append(
        oMi["middle_cervical_Y"].inverse().rotation
        @ (markers_global["LFHD"] - oMi["middle_cervical_Y"].translation)
    )
    local_values.append(
        oMi["middle_cervical_Y"].inverse().rotation
        @ (markers_global["RFHD"] - oMi["middle_cervical_Y"].translation)
    )
    local_values.append(
        oMi["middle_cervical_Y"].inverse().rotation
        @ (markers_global["LBHD"] - oMi["middle_cervical_Y"].translation)
    )
    local_values.append(
        oMi["middle_cervical_Y"].inverse().rotation
        @ (markers_global["RBHD"] - oMi["middle_cervical_Y"].translation)
    )
    local_values.append(
        oMi["middle_thoracic_Y"].inverse().rotation
        @ (markers_global["C7"] - oMi["middle_thoracic_Y"].translation)
    )
    local_values.append(
        oMi["middle_abdomen_Y"].inverse().rotation
        @ (markers_global["T10"] - oMi["middle_abdomen_Y"].translation)
    )
    local_values.append(
        oMi["middle_thoracic_Y"].inverse().rotation
        @ (markers_global["CLAV"] - oMi["middle_thoracic_Y"].translation)
    )
    local_values.append(
        oMi["middle_abdomen_Y"].inverse().rotation
        @ (markers_global["STRN"] - oMi["middle_abdomen_Y"].translation)
    )
    local_values.append(
        oMi["middle_thoracic_Y"].inverse().rotation
        @ (markers_global["RBAK"] - oMi["middle_thoracic_Y"].translation)
    )
    local_values.append(
    ...
    local_values.append(
        oMi["left_elbow_Y"].inverse().rotation
        @ (markers_global["LELB"] - oMi["left_elbow_Y"].translation)
    )
    local_values.append(
        oMi["left_wrist_X"].inverse().rotation
        @ (markers_global["LWRA"] - oMi["left_wrist_X"].translation)
    )
    local_values.append(
        oMi["left_wrist_X"].inverse().rotation
        @ (markers_global["LWRB"] - oMi["left_wrist_X"].translation)
    )
    local_values.append(
        oMi["left_wrist_X"].inverse().rotation
        @ (markers_global["LFIN"] - oMi["left_wrist_X"].translation)
    )
    local_values.append(
    ...
    local_values.append(
        oMi["right_elbow_Y"].inverse().rotation
        @ (markers_global["RELB"] - oMi["right_elbow_Y"].translation)
    )
    local_values.append(
        oMi["right_wrist_X"].inverse().rotation
        @ (markers_global["RWRA"] - oMi["right_wrist_X"].translation)
    )
    local_values.append(
        oMi["right_wrist_X"].inverse().rotation
        @ (markers_global["RWRB"] - oMi["right_wrist_X"].translation)
    )
    local_values.append(
        oMi["right_wrist_X"].inverse().rotation
        @ (markers_global["RFIN"] - oMi["right_wrist_X"].translation)
    )
    local_values.append(
        oMi["middle_lumbar_X"].inverse().rotation
        @ (markers_global["LASI"] - oMi["middle_lumbar_X"].translation)
    )
    local_values.append(
        oMi["middle_lumbar_X"].inverse().rotation
        @ (markers_global["RASI"] - oMi["middle_lumbar_X"].translation)
    )
    local_values.append(
        oMi["middle_lumbar_X"].inverse().rotation
        @ (markers_global["LPSI"] - oMi["middle_lumbar_X"].translation)
    )
    local_values.append(
        oMi["middle_lumbar_X"].inverse().rotation
        @ (markers_global["RPSI"] - oMi["middle_lumbar_X"].translation)
    )
    local_values.append(
        oMi["left_hip_Y"].inverse().rotation
        @ (markers_global["LTHI"] - oMi["left_hip_Y"].translation)
    )
    local_values.append(
    ...
    local_values.append(
        oMi["left_ankle_X"].inverse().rotation
        @ (markers_global["LANK"] - oMi["left_ankle_X"].translation)
    )
    local_values.append(
        oMi["left_ankle_X"].inverse().rotation
        @ (markers_global["LHEE"] - oMi["left_ankle_X"].translation)
    )
    local_values.append(
        oMi["left_ankle_X"].inverse().rotation
        @ (markers_global["LTOE"] - oMi["left_ankle_X"].translation)
    )
    local_values.append(
        oMi["right_hip_Y"].inverse().rotation
        @ (markers_global["RTHI"] - oMi["right_hip_Y"].translation)
    )
    local_values.append(
    ...
    local_values.append(
        oMi["right_ankle_X"].inverse().rotation
        @ (markers_global["RANK"] - oMi["right_ankle_X"].translation)
    )
    local_values.append(
        oMi["right_ankle_X"].inverse().rotation
        @ (markers_global["RHEE"] - oMi["right_ankle_X"].translation)
    )
    local_values.append(
        oMi["right_ankle_X"].inverse().rotation
        @ (markers_global["RTOE"] - oMi["right_ankle_X"].translation)
    )
