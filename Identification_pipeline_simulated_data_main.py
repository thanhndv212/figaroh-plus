...
def main():
    ...
    # this does not work so we will set the missing parameters here
    ...
    for ii in range(robot.model.nq):
        accelerationLimit[ii] = params_settings[
            "ddq_lim_def"
    ...
    data_load = np.loadtxt(
        "results/trajectory/"
        + model.name
        + "/joint_by_joint_trapezoidal_trajectory.txt",
        delimiter=",",
    )
    ...
    q_filt = low_pass_filter_data(q[:, 0], params_settings)
    ...
    phi_base = np.matmul(np.linalg.pinv(W_base), tau_meas)
    # Add a bit of noise
    # phi_base = phi_base + np.random.uniform(low=-1, high=1, size=(len(phi_base)))
    tau_est = np.matmul(W_base, phi_base)
    # phi_base_weigthed = weigthed_least_squares(
    # robot, phi_base, W_base, tau_meas, tau_est, param
    # )
    ...
    # coeff = 0.01 * np.ones((2 * param["NbH"] + 1, model.nq)) + np.random.uniform(
    # low=-0.1, high=1, size=((2 * param["NbH"] + 1, model.nq))
    # )
    ...
    # W_b_tot, numrank_W_tot, params_rsorted_tot, phi_b_l = double_QR(
    # tau, W_tot, params_tot
    # )
